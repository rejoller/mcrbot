...
async def init_redis():
    ...
    try:
        redis = Redis.from_url('redis://default:CQLSunqzyDrqxToM2DyLCjpD0iSrmTnj@redis-15093.c304.europe-west1-2.gce.cloud.redislabs.com:15093', port=11158, password='aoOlWYyOKYFXMozbi6b24ja0A011RuEl', encoding="utf-8", decode_responses=True)
...
async def load_values(spreadsheet, redis):
    ...
    try:
        ...
        for row in rows:
            if len(row) > 3:
                town_name = str(row[0]).lower()
                unique_key = str(row[4]).lower()
                pipeline.sadd(f"town:{town_name}", unique_key)  # Добавление в множество без await
                pipeline.set(f"data:{unique_key}", json.dumps(row))  # Сохранение данных без await

        await pipeline.execute()  # Выполнение всех накопленных команд пайплайна
...
if __name__ == "__main__":
    # В Python 3.7+ лучше использовать asyncio.run для запуска основной программы

    executor.start_polling(dp, on_startup=on_startup)
    async def main():

        try:
            # Здесь может быть ваш код, например, запуск телеграм-бота
