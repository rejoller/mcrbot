        ...
        try:
            async with client_session.get(GOSUSLUGI_URL, ssl=False, timeout=5) as response:
                if response.status == 200:
                    ...
                    ucn2025_df.columns = ['city_name_from_gosuslugi', 'number_of_votes_ucn2025']

                    for index, row in ucn2025_df.iterrows():
                        # Поиск city_id по city_name_from_gosuslugi
                        city_id_query = select(Cities.city_id).where(Cities.city_name_from_gosuslugi == row['city_name_from_gosuslugi'])
